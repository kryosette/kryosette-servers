...
{
  roa_table_t *table = malloc(sizeof(roa_table_t));
  // error handling
  table->count = 0;
  table->capacity = 32;
  table->ipv4_count = 0;
  ...
  table->entries = calloc(table->capacity, sizeof(roa_entry_t));
  ...
  table->add_entry = roa_table_add;
  table->find_entry = roa_table_find;
...
int roa_table_add(const roa_table *table, const char *ip_prefix_str, uint8_t prefix_len, uint8_t max_prefix_len, uint32_t asn, const char *source)
{
  ...
  */
  ...
  entry->info.valid_until = time(NULL) + (365 * 24 * 3600);
