...
// Явно объявляем структуру ARP-заголовка, так как она может быть не определена в стандартных заголовках
struct arphdr {
...
#define ARPOP_REPLY 2 // Код ответа ARP

// Структура для доверенной записи
typedef struct {
    uint32_t ip;        // IP в network byte order
    unsigned char mac[6]; // MAC-адрес
} trusted_pair_t;

// Белый список (доверенные пары IP-MAC). Инициализируем в main.
trusted_pair_t *trusted_list;
size_t num_trusted = 0;

// Функция для проверки, является ли ARP-пакет подозрительным
int is_arp_poisoning(const struct arphdr *arp_hdr, const unsigned char *src_mac) {
    // Проверяем, это ARP-ответ?
    if (ntohs(arp_hdr->ar_op) != ARPOP_REPLY) {
        return 0; // Это не ответ, пропускаем
    }

    // Проходим по белому списку
    for (size_t i = 0; i < num_trusted; i++) {
        // Если IP из пакета есть в белом списке
        if (memcmp(arp_hdr->__ar_sip, &trusted_list[i].ip, sizeof(uint32_t)) == 0) {
            // Но MAC-адрес отправителя НЕ совпадает с доверенным
...
int main() {
    // Инициализируем белый список вручную (можно вынести в конфиг)
    num_trusted = 2;
