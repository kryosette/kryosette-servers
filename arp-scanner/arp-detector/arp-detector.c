...
int load_trusted_base(const char *filename) {
    ...
    if (!file) {
        perror("Could not open trusted base file");
        return -1;
    }
    ...
    printf("Loading trusted ARP base...\n");

    while (fgets(line, sizeof(line), file)) {
        if (line[0] == '#' || line[0] == '\n') continue;

        if (sscanf(line, "%16s %17s", ip_str, mac_str) == 2) {
            struct arp_entry new_entry;

            if (inet_pton(AF_INET, ip_str, &new_entry.ip) != 1) {
                fprintf(stderr, "Invalid IP in trusted base: %s\n", ip_str);
                continue;
            }

            struct ether_addr *mac = ether_aton(mac_str);
            if (!mac) {
                fprintf(stderr, "Invalid MAC in trusted base: %s\n", mac_str);
                continue;
            }
            new_entry.mac = *mac;

            if (entry_count < MAX_ENTRIES) {
                arp_table[entry_count++] = new_entry;
                printf("Trusted: %s -> %s\n", ip_str, mac_str);
            }
        }
    }
    ...
    fclose(file);
    printf("Trusted base loaded. %d entries.\n", entry_count);
    return 0;
}